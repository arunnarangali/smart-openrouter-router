    ...
    def get_free_models(self, api_key):
        with self._lock:
            ...
            try:
                ...
                free = filter_free_models(data.get("data", []))
...
def analyze_scenario(messages):
    text = " ".join(_content_to_text(msg.get("content")) for msg in messages)
    result = analyze_prompt_text(text)
    log.info("Scenario scores: %s; selected=%s", result["scenario_scores"], result["detected_scenario"])
    return result
